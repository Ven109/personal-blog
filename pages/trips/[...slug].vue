...
const { data: page } = await useAsyncData(path.value ,async () => await queryContent(path.value).only(['trip']).findOne(), { watch: [path]});
const { data: surround } = await useAsyncData(path.value, async () => await queryContent('/trips')
    .where({ trip: page?.value ? page.value.trip : ''})
    .only(['_path', 'title'])
    .sort({ date: 1})
...
const prev = computed(() => surround.value && surround.value.length === 2 ? surround.value[0] : null);
const next = computed(() => surround.value && surround.value.length === 2 ? surround.value[1] : null);
